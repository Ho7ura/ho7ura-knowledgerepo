...
/*
 * Electrochemistry workflow core.
 *
 * This module translates parsed host commands into potentiostat actions,
 * samples the analog feedback path, and streams measurement points back to
...
Current_Range_Type Current_Range = RANGE_2;  // 默认 10kΩ 档
...
/* Clamp requested voltages to the analog front-end output window. */
...
/* Report that the current sweep is finished and the device is back in idle mode. */
...
/* Seed each mode with safe defaults so power-on state is deterministic. */
void InitParams() {
...
/*
 * Run a chronoamperometry / IT experiment.
 *
 * TIM2 drives the sampling cadence, while the loop tracks paused time so the
 * requested duration only counts active experiment time.
 */
...
/* Execute a cyclic voltammetry sweep after validating range and waveform inputs. */
...
/* Reset cached plot values before a fresh voltammetry run starts. */
...
/* Central dispatcher used by main.c after each payload refresh. */
...
/*
 * Convert raw 16-bit payload words into the global parameter structs consumed
 * by the execution functions below.
 */
...
/*
 * Map a signed millivolt request into the DAC code expected by the analog
 * front end. The output is centered around mid-scale.
 */
...
/* Drive the ADG704 selection lines for the requested measurement range. */
void setCurrentRange(Current_Range_Type range) {
    if (Current_Range == range) return;
    Current_Range = range;

    // ADG704 真值表: A1(PA2) A0(PA1) EN(PB12)
    // RANGE_1 (1kΩ):   A1=0 A0=0 EN=1
    // RANGE_2 (10kΩ):  A1=0 A0=1 EN=1
    // RANGE_3 (100kΩ): A1=1 A0=0 EN=1
    // RANGE_4 (1MΩ):   A1=1 A0=1 EN=1
...
/* Return the active transimpedance resistor value for current conversion. */
...
/* Sample the ADC path and convert the measured drop into current units. */
...
/* Serialize one measurement point into the compact host transport payload. */
...
/*
 * Apply the next potential step, wait for the requested scan interval,
 * then sample and forward the resulting current.
 */
